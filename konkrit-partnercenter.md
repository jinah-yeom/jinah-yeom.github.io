...
**소제목:** 수기 운영이 만든 반복 문의와 보이지 않는 판매 현황
...
**소제목:** 화면보다 구조를 먼저, 스펙에 존재 이유를 담는 설계
...
**소제목:** 명확한 정보 구조(IA)가 구축되어야 비로소 사용자 경험이 정교해집니다
...
Solution은 tint title 하나로 통합. 아래 세 블록은 각각 소제목(h3)과 본문만 노출하며 "Solution (…)" 형태의 블록 제목은 쓰지 않는다.

### 예외까지 고려한 안정적인 인증 경험 설계
...
`[에셋]` 회원가입·로그인 플로우 프레임 8장, 2열 그리드 (제너럴 케이스 중심)

### 유형별 입력 흐름까지 정의한 상품 등록 플로우 구축

상품 등록은 유형 선택부터 기본 정보·옵션 정보 입력, 등록 완료와 승인 요청까지 단계가 길고, 선택한 유형에 따라 입력 항목이 달라지는 플로우였습니다. 파트너센터의 상품은 사용 방식에 따라 두 유형으로 나뉩니다.

> **바우처형** : 사용 기간 내 원하는 시점에 현장에 방문해 사용하는 NFT 상품. 예: PC방 4시간 이용권.

> **날짜·좌석 선택형** : 사용 기간 내 정해진 날짜의 회차를 선택해 사용하는 NFT 상품. 예: 콘서트 티켓.
...
`[에셋]` 상품 등록 플로우 프레임: 바우처형 4장 2×2 + 날짜선택형 4장 2×2, 유형 라벨 각 1 (제너럴 케이스 중심)

### 명확한 상품 관리 경험을 위한 직관적 정보 구조 설계
...
`[에셋]` 상품 목록 플로우 프레임 8장, 2열 그리드 (제너럴 케이스 중심)

---

## Collaboration

**소제목:** 스펙 위에서 질문하고 답하는 협업 구조
...
`[에셋]` 협업 코멘트 캡처 2장, 세로 스택(위: konkrit-collaboration-01 상품 파일 코멘트 스레드 / 아래: konkrit-collaboration-02 로그인 파일 코멘트 상세)

---

## Outcome

**흩어진 판매·프로젝트 정보를 한눈에 파악**
...
**신규 페이지 확장에 대응 가능한 재사용성 높은 UI 기반 마련**
