# Keeper Admin 소모품 구매 케이스 초안 (덱 → v2 템플릿 매핑)

> 원문은 전부 덱 텍스트 기반 — 프레젠테이션 문장을 웹 프로즈로만 이어붙임. `[확인 필요]` 해소 후 확정.

---

## 1. Hero
- eyebrow: KEEPER ADMIN
- 제목: Keeper Admin 소모품 구매
- 리드: 외부 서비스에서 진행되던 소모품 구매를 Admin 내부로 통합했습니다. 탐색부터 결제까지 끊기지 않는 흐름을 설계해, 외부 이탈 없이 구매가 완결되는 구조를 만들었습니다.
- 대표 이미지: 덱 표지의 화면 콜라주 [export 필요]

## 2. Meta
| 항목 | 값 |
|---|---|
| Timeline | 2025.09 – 2025.11 |
| Product | Keeper Admin (Web · B2B) |
| Team | PD 1 · FE 1 · BE 1 |
| Role | 구매 흐름·의사결정 정책 정의 · IA/UX 설계 · 화면 단위 명세 |
| Tools | Figma, Jira |
...
- 미디어: as-is 플로우 (소모품 부족 감지 → 플랫폼 외 이탈 → 가격·배송 비교 반복 → 구매 지연) [덱 슬라이드 재구성 or export]
...
## 6. Approach (카드 — 가설 도입 문단 + 3~4장)
도입: 가설은 하나였습니다. 고객의 소모품 구매 행동을 촉진하는 핵심 동기는 '즉시성'이며, 이를 플랫폼 내부에서 해결할 수 있을 때 구매 전환율과 운영 효율이 함께 상승할 것이다. 이 가설에 따라 화면 이동을 최소화하고 구매 흐름이 한 번에 이어지는 구조로 정책과 IA를 정의했습니다.

- [구매 흐름] **즉시성을 살리는 Seamless Flow**: 탐색에서 결제까지 화면 이동 최소화 / '바로 구매' 경험이 빠르게 실행되는 흐름 구조화
- [정보 구조] **AirSupply 정책·데이터를 흡수한 IA 재정의**: 외부 서비스의 API 구조·옵션 정책·배송지 제약 분석 / Keeper Admin 안에서 자연스럽게 쓰이도록 재설계
- [운영 정책] **운영팀 워크플로우까지 고려한 정책 설계**: 구매·정산 분산으로 생기던 비효율 제거 / 단계별로 정보가 명확히 전달되는 구조
- [전체 공통] **화면보다 명세 먼저**: 정책 문서·의사결정 플로우차트·예외 케이스 정책 선행 / Jira 기반 화면 단위 명세로 해석 차이 제거 (포함 확정)

## 7. Solution 1 — 계정 연동
### 구매의 시작점을 한 화면 안에서 끝냈습니다
...
- 미디어: 연동 판단 / 케이스 라우팅 / 로그인 플로우 [export]

## 8. Solution 2 — 상품 탐색
### 외부 상품을 Keeper의 탐색 방식으로 다시 담았습니다
...
- 미디어: 상품 리스트 / 카드 정보 구조·CTA / 빈 상태·오류 [export]

## 9. Solution 3 — 결제 플로우
### 결제를 확인 몇 번의 일로 줄였습니다
...
- 미디어: 배송지 선택 / 수량·옵션 변경·재고 검증 / 결제 확인·제출 [export]

## 10. Solution 4 — 주문 요약
### 구매 직후에 다음 행동이 보이게 했습니다
...
- 미디어: 주문 현황 / 주문 상세 / 오류 상태 [export]

## 11. Interaction Detail — 예외·빈 상태
...
- 미디어: 명세 문서 3종 (정책·플로우차트 포함 조합 이미지) [export]
...
## 확정 사항
1. 라우트: `/work/keeper-supplies`
2. 홈 카드 태그: `["B2B Admin", "Procurement"]` (메뉴 생략 케이스)
3. AirSupply 실명 공개
4. 화면 데이터: 실데이터 그대로 (더미 치환 없음)
5. Reflection: 위 14번 초안 반영됨
6. 미디어: Solution당 개별 3장 export
