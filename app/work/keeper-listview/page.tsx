...
const SOLUTIONS: SolutionBlockProps[] = [
  ...
  {
    headline: "지점 하나만 고르던 필터를 지점 → 동 → 층 3단계로 확장했다",
    paragraphs: [
      "기존에는 지점만 필터링할 수 있었습니다. 어반스테이 명동을 골라도 909동 9층의 업무를 찾으려면 결국 눈으로 훑어야 했습니다. 지점 → 동 → 층의 3단계 계층 구조를 Bottom Sheet 하나에서 탐색하도록 확장했습니다. 별도 화면 이동 없이 시트 안에서 범위를 좁혀가는 구조라, 필터를 거는 행동 자체의 비용이 낮습니다. 키퍼는 자신의 동선에 맞는 공간 범위를 몇 번의 탭으로 좁히고, 그 범위의 업무만 봅니다.",
    ],
    media: [{ label: "as-is/to-be 비교 — 지점 → 동 → 층 필터", ratio: "wide" }],
  },
  {
    headline: "필터와 정렬이 하나의 탐색 흐름으로 이어지게 했다",
    paragraphs: [
      "필터와 정렬을 각각의 기능이 아니라 하나의 탐색 흐름으로 설계했습니다. 업무 유형·상태 조건의 필터링과 정렬 기준이 날짜 이동과 충돌하지 않도록, 정책 단계에서 정의한 유지·초기화 규칙을 그대로 UI에 반영했습니다. 적용 중인 필터는 리스트 상단에 항상 노출해, 지금 보고 있는 리스트가 어떤 조건의 결과인지 놓치지 않게 했습니다. 업무 탐색–조건 선택–날짜 비교가 하나의 유기적 흐름으로 연결됩니다.",
    ],
    media: [{ label: "필터·정렬 플로우", ratio: "wide" }],
  },
  {
